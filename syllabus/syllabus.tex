...
Esta semana es la entrada al curso. Cubre la motivación detrás de elegir R, la instalación y configuración del entorno de trabajo, los primeros conceptos del lenguaje (asignación, vectores, tipos), y la posición del curso sobre el uso de modelos de lenguaje.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item \textbf{Uso responsable de modelos de lenguaje en el curso.} Diferencia operativa entre usar un LLM como \blue{chatbot} (pegar pregunta, copiar respuesta) y como \orange{agente integrado en el proyecto} (lee archivos, propone cambios, ejecuta comandos con el humano en el \textit{loop} ---por ejemplo, Claude Code o el modo agente de Cursor). Depender mal de un LLM \textbf{atrofia la capacidad individual} de razonar sobre código; usado como agente puede acelerar el trabajo de quien ya domina los fundamentos. \textbf{Política del curso:} agentes permitidos en el proyecto integrador; \textit{no} permitidos en los ejercicios de clase. El estudiante debe ser capaz de leer y modificar cualquier línea que entregue.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item \textbf{Listas.} Contenedores heterogéneos; construcción con \texttt{list()}; nombres; acceso con \texttt{\$} y con \texttt{[[]]}; diferencia operativa con vectores atómicos; aplicación típica: agrupar resultados heterogéneos de un análisis.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item \textbf{Consumo de APIs.} Más allá de archivos, los datos también se obtienen de servicios web. Introducción a \texttt{httr2} como cliente HTTP moderno: construir \textit{requests} con \texttt{request()}, ejecutar con \texttt{req\_perform()}, parsear respuestas JSON con \texttt{resp\_body\_json()}, autenticación por \textit{token} en \textit{headers}. Caso de uso central: la \blue{API SIE de Banxico} para series macroeconómicas (tipo de cambio, INPC, tasas de interés). Aplicación al proyecto integrador: complementar microdatos del INEGI con contexto macro de Banxico, habilitando preguntas que cruzan ambas fuentes.
...
\noindent\textbf{Objetivos de aprendizaje}
\begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item Importar microdatos desde archivos (CSV, Excel) verificando \textit{encoding}, tipos y representación de \texttt{NA}.
...
Esta semana cierra la Parte I del curso. Cubre las herramientas que convierten un \textit{script} suelto en un proyecto reproducible: estructura de directorios, control de versiones con Git y captura de dependencias con \texttt{renv}. La estructura propuesta se basa en la \blue{convención utilizada en proyectos profesionales reales del profesor}, simplificada para el contexto académico. La meta práctica al cierre: que cualquier persona pueda clonar el repositorio del proyecto integrador, instalar las dependencias, correr el \textit{master script} y obtener exactamente los mismos outputs.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item \textbf{Sistema tidyverse y filosofía.} Qué es el tidyverse y qué paquetes lo componen (\texttt{dplyr}, \texttt{tidyr}, \texttt{ggplot2}, \texttt{readr}, \texttt{tibble}, \texttt{purrr}, \texttt{forcats}, \texttt{stringr}, \texttt{lubridate}). La idea de un \textbf{sistema coherente} ---API consistente, gramática compartida--- frente a la alternativa de paquetes sueltos. Convención del curso: cargar con \texttt{pacman::p\_load(tidyverse)}.
    ...
    \item \textbf{\textit{Non-Standard Evaluation} (mención).} Por qué \texttt{filter(df, x > 5)} funciona sin escribir \texttt{df\$x} ni cotizar \texttt{"x"}. Introducción brevísima al concepto, suficiente para anticipar las limitaciones que aparecen al escribir funciones que reciben nombres de columnas como argumentos. El tratamiento operativo (\texttt{\{\{~\}\}}, \textit{tunneling}) se difiere a la Semana 9.
\end{itemize}
...
\begin{enumerate}[leftmargin=*, itemsep=1pt]
    ...
    \item Identificar si un dataset está en formato \textit{tidy} y, en caso contrario, dimensionar el \textit{reshape} requerido.
    \item Componer secuencias de verbos \texttt{dplyr} conectados por el \textit{pipe} para resolver preguntas de manipulación de datos sobre microdatos de encuesta.
...
Esta semana completa el conjunto de operaciones que el estudiante necesita para transformar microdatos crudos en un dataset \textit{tidy} listo para análisis: cierra el tema de \textit{tidy data} iniciado en la Semana 5 con los \textit{pivots}, introduce los \textit{joins} entre tablas, formaliza el manejo avanzado de \texttt{NA}, y cubre la construcción de variables categóricas derivadas. Al cierre de la semana, el estudiante tiene casi todo lo necesario para armar el pipeline E$\to$T del proyecto integrador (la Semana 7 añade \textit{strings} y \textit{regex}, también frecuentes en la limpieza).

\textbf{Subtemas:}

\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item \textbf{Pivots.} \texttt{pivot\_longer()} y \texttt{pivot\_wider()} para \textit{reshape} entre formato ancho y largo. Argumentos clave: \texttt{cols}, \texttt{names\_to}, \texttt{values\_to} (\textit{longer}); \texttt{names\_from}, \texttt{values\_from} (\textit{wider}). Casos típicos en encuestas: variables-columna con sufijos por año o módulo (ENSU, ENIGH). Round-trip \textit{long} $\leftrightarrow$ \textit{wide} como prueba de identidad.
    \item \textbf{Joins entre tablas.} \textit{Mutating joins} (\texttt{left\_join}, \texttt{right\_join}, \texttt{inner\_join}, \texttt{full\_join}) frente a \textit{filtering joins} (\texttt{semi\_join}, \texttt{anti\_join}). Concepto de \textit{key} y sintaxis preferida con \texttt{join\_by()}. Errores comunes: cardinalidad inesperada (\textit{many-to-many}), \textit{keys} duplicadas, \textit{joins} silenciosos sobre columnas mal pareadas. Como mitigación introducimos \blue{\texttt{tidylog}}, que intercepta operaciones de \texttt{dplyr} y \texttt{tidyr} y reporta en consola cuántas filas se afectaron, cuántas no encontraron \textit{match} y cuándo aparecen \texttt{NA} nuevos ---convierte fallas silenciosas en mensajes explícitos. Aunque se introduce con los \textit{joins}, aplica a todos los verbos vistos esta semana y la anterior.
    ...
    \item \textbf{Construcción de variables categóricas.} \texttt{if\_else()} vectorizado para condiciones binarias y \texttt{case\_when()} para reglas múltiples mutuamente excluyentes. Mención a \texttt{forcats} para reordenar y recodificar factores (sin profundizar; aparece según necesidad en el proyecto integrador).
\end{itemize}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item \textbf{\texttt{stringr}: API consistente.} El patrón \texttt{str\_*()} ofrece un sistema coherente análogo al de \texttt{dplyr}. Funciones \textit{core}: detección (\texttt{str\_detect}), extracción (\texttt{str\_extract}, \texttt{str\_match}), reemplazo (\texttt{str\_replace}, \texttt{str\_replace\_all}), división (\texttt{str\_split}), \textit{trimado} (\texttt{str\_trim}, \texttt{str\_squish}), \textit{padding} (\texttt{str\_pad}), conversión de capitalización (\texttt{str\_to\_lower}, \texttt{str\_to\_upper}, \texttt{str\_to\_title}), longitud (\texttt{str\_length}). Argumentos vectorizados por construcción.
...
\noindent\textbf{Objetivos de aprendizaje}
\begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item Manipular vectores de \textit{strings} con la API de \texttt{stringr}, distinguiendo cuándo basta una operación de \textit{base R} y cuándo se requiere el sistema.
    ...
    \item Limpiar inconsistencias textuales típicas en microdatos de encuesta (capitalización, espacios extra, formatos heterogéneos) combinando \texttt{stringr} con \textit{regex}.
...
Esta semana es la transición del estudiante de \textit{usuario} a \textit{autor} de código. Las funciones son la unidad fundamental de reuso y la pieza que permite refactorizar el ETL del proyecto integrador (\orange{Checkpoint 3}, al cierre de la semana siguiente). Cubrimos la anatomía de una función, el modelo de \textit{scoping} con el que R resuelve nombres, las herramientas para producir y capturar errores, las técnicas para diagnosticarlos cuando ocurren, y el operador \textit{embrace} \texttt{\{\{~\}\}} ---que cierra la inquietud sembrada en la Semana 5 sobre cómo escribir funciones que interactúan con \texttt{dplyr}.
...
Esta semana cierra la Parte III del curso. Cubre la \blue{programación funcional} como paradigma: tratar funciones como datos para iterar sobre colecciones sin escribir \textit{loops}. La semana es la natural continuación de S8 (vectorización) y S9 (funciones): donde S8 enseña a evitar \textit{loops} sobre operaciones \textit{built-in}, S10 enseña a evitar \textit{loops} sobre funciones propias. Coincide con la entrega del \orange{Checkpoint 3} del proyecto integrador: refactor del ETL en funciones reutilizables, orquestadas por un \textit{master script}.
...
\begin{enumerate}[leftmargin=*, itemsep=1pt]
    ...
    \item Paralelizar una iteración con \texttt{furrr}/\texttt{future} cuando se justifica, anticipando las limitaciones de memoria que impone el modelo multi-proceso de R y eligiendo el \textit{backend} apropiado.
\end{enumerate}
...
Esta semana cierra la Parte IV del curso. Cubre las herramientas para conectar R con bases de datos relacionales: SQL como lenguaje, \texttt{DBI} como API de conexión y \texttt{dbplyr} como puente con la sintaxis de \texttt{dplyr}. Coincide con el \orange{Checkpoint 4} del proyecto integrador: conectar el \textit{Load} del ETL a una base local y aplicar análisis estadísticos sobre esos datos. La meta práctica al cierre: que el estudiante pueda mover datos entre archivos, memoria y BD, y elegir con criterio cuál es la mejor representación para cada momento del proyecto.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item \textbf{SQL básico-intermedio.} \texttt{SELECT}, \texttt{FROM}, \texttt{WHERE}, \texttt{ORDER BY}, \texttt{LIMIT}. \texttt{JOIN} (\texttt{INNER}, \texttt{LEFT}, \texttt{RIGHT}, \texttt{FULL}). \texttt{GROUP BY} con funciones agregadoras (\texttt{COUNT}, \texttt{SUM}, \texttt{AVG}, \texttt{MIN}, \texttt{MAX}). Operadores: \texttt{LIKE}, \texttt{IN}, \texttt{BETWEEN}, \texttt{IS NULL}. \textit{Common Table Expressions} con \texttt{WITH} para queries legibles que se construyen en pasos. Mención a \textit{subqueries}. Statements de creación: \texttt{CREATE TABLE} e \texttt{INSERT} ---solo lo necesario para el \textit{Load} del proyecto.
    \item \textbf{\texttt{DBI}: la API de conexión.} \texttt{dbConnect()}/\texttt{dbDisconnect()} para abrir y cerrar conexiones; conveniencia de envolver en \texttt{tryCatch()} para garantizar el cierre. Operaciones de tabla: \texttt{dbWriteTable()}, \texttt{dbReadTable()}, \texttt{dbListTables()}, \texttt{dbListFields()}. Ejecución de queries: \texttt{dbGetQuery()} cuando se espera resultado tabular; \texttt{dbExecute()} cuando no (creación, inserción, actualización). Parametrización segura: \texttt{glue\_sql()} para interpolar valores en queries sin riesgo de \textit{SQL injection}. Para los alcances del curso, una conexión directa por sesión es suficiente; \textit{connection pooling} con el paquete \texttt{pool} solo se menciona como herramienta de producción para Shiny.
    ...
    \item \textbf{Aplicación al proyecto integrador.} Conectar el paso \textit{Load} del ETL a una base SQLite local en \texttt{output/proyecto.sqlite}. Diseño básico de \textit{schema}: tabla principal de la encuesta, tablas de catálogos (entidades, municipios, recodificaciones), tabla de series macro de Banxico (si aplica). Criterio de decisión: ¿\texttt{.rds} o SQLite? \texttt{.rds} basta si los datos son chicos y se cargan completos cada vez; SQLite gana cuando hay queries repetidas, múltiples consumidores (Shiny, reporte) o crecimiento previsible. \textit{Preview} de S15: el Shiny consumirá esta misma BD como fuente.
\end{itemize}

\medskip
\noindent\textbf{Objetivos de aprendizaje}
\begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item Reconocer cuándo un proyecto de datos justifica una BD y elegir entre las opciones disponibles (SQLite, duckdb, PostgreSQL, MySQL) con criterio.
    \item Escribir queries SQL básico-intermedias que cubran el grueso de las necesidades de análisis (\texttt{SELECT}/\texttt{JOIN}/\texttt{GROUP BY}/agregadoras/\texttt{WITH}).
    ...
    \item Integrar el \textit{Load} del ETL a una base SQLite local como parte del proyecto integrador, justificando la decisión frente a la alternativa de archivos serializados.
\end{enumerate}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item \textbf{Integración al pipeline y manejo de secretos.} El \textit{script} de presentación como paso final del \texttt{master.R} del proyecto: tras correr el ETL y los análisis, se llama \texttt{source('pre/04-presentacion.R')}. Credenciales (\textit{Service Account} JSON, tokens) viven fuera del repositorio: \texttt{.gitignore} entrada para \texttt{credenciales.json} y \texttt{.env}, en línea con la convención introducida en S4. Conexión con el \orange{Checkpoint 5} del proyecto integrador: el reporte automatizado vía \texttt{officer} es uno de los dos entregables finales (el otro es el tablero Shiny, S15).
\end{itemize}
...
\begin{enumerate}[leftmargin=*, itemsep=1pt]
    ...
    \item Diseñar canales de automatización \textit{low-code} en los que carpetas de Drive y bandejas de Gmail funcionan como buzones de insumos/outputs entre el código y el equipo.
\end{enumerate}
...
Esta es la última semana del curso. Cierra la Parte V y coincide con la entrega final del proyecto integrador (\orange{Checkpoint 5}: tablero Shiny + reporte automatizado). El tema: cómo incorporar modelos de lenguaje (LLMs) al pipeline de datos como una herramienta más, en lugar de tratarlos como un servicio externo que rompe el flujo. La meta práctica es que el estudiante pueda mover tareas que habitualmente se delegan manualmente a un \textit{chatbot} ---extraer información estructurada de un párrafo, clasificar texto, generar un resumen, normalizar nombres--- al interior del pipeline, donde son \blue{auditables, reproducibles y se ejecutan sin intervención humana}.
