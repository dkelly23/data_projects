...
% ==============================================================================
\section{Hilo conductor: la ENSU}
% ==============================================================================
...
El conjunto elegido es la \textbf{Encuesta Nacional de Seguridad Pública Urbana (ENSU)} del INEGI, de levantamiento trimestral. Las razones:

\begin{itemize}[leftmargin=*, itemsep=1pt]
    \item \textbf{Tamaño manejable.} Los archivos son ligeros y la lectura es inmediata en cualquier máquina.
    \item \textbf{Varios módulos relacionables.} Cuestionario básico, módulo sociodemográfico y módulo de vivienda comparten identificadores, de modo que el ejercicio de \textit{join} es sustantivo y no artificial.
    \item \textbf{Varios levantamientos.} La periodicidad trimestral justifica la acumulación temporal y el manejo de nombres de archivo variables.
    \item \textbf{Datos crudos representativos.} Variables categóricas codificadas numéricamente, catálogos en descriptores externos, valores faltantes y no respuesta: el material típico de la limpieza de datos.
\end{itemize}

Los ejemplos en clase y los ejercicios del bloque de práctica se construyen sobre estos archivos a lo largo de las seis sesiones.
...
\begin{itemize}[leftmargin=*, itemsep=1pt]
    \item \textbf{Sesión 3.} \texttt{dplyr} I: \textit{pipe}, verbos básicos y sistema tidyverse.
...
\practica{Instalación verificada de R y Positron. Creación del repositorio del curso con la estructura de directorios convenida, \texttt{.gitignore} inicial y primer \textit{commit} sincronizado a GitHub. Primer \textit{script}: construcción de vectores, inspección de tipos y cálculo de proporciones sobre condiciones lógicas.}
...
El uso se restringe deliberadamente a las funciones base de R y a los paquetes de importación. La decisión es pedagógica: el estudiante reconoce qué ofrece el lenguaje por sí mismo antes de incorporar el tidyverse (Sesiones 3--4) y \texttt{ggplot2} (Sesión 6).
...
\practica{Importación de un trimestre de la ENSU desde archivo plano, con verificación de \textit{encoding} y tipos contra el descriptor técnico. Inspección post-importación, corrección de las variables mal inferidas y primera exploración descriptiva y gráfica.}
...
\sesion{3}{dplyr I: pipe, verbos básicos y sistema tidyverse}

El contenido subsiguiente del curso ---limpieza avanzada, programación y visualización--- presupone fluidez con \texttt{dplyr} y con el \textit{pipe} como mecanismo de composición. La meta de la sesión es alcanzar dicha fluidez: traducir una pregunta sobre un \texttt{tibble} en una secuencia de verbos, y leer código tidyverse de terceros sin interrupciones.
...
\practica{Traducción de un conjunto de preguntas sustantivas sobre la ENSU a secuencias de verbos: filtrado por ciudad y trimestre, construcción de indicadores con \texttt{mutate()} y agregación por grupo con \texttt{group\_by()} y \texttt{summarize()}.}
...
\practica{Construcción del ETL de la ENSU: \textit{join} entre módulos por identificador de vivienda, \textit{reshape} de las baterías de preguntas a formato largo, recodificación de categóricas con \texttt{case\_when()} y limpieza de los catálogos de texto con \texttt{stringr}.}
...
\practica{Refactor del ETL de la Sesión 4 en funciones de un solo propósito: una que lea y valide un trimestre, otra que recodifique las categóricas, otra que produzca un resumen por ciudad recibiendo la columna como argumento. Validación de argumentos y captura de fallas con \texttt{tryCatch()}.}
...
\practica{Cierre del pipeline: lectura de todos los trimestres disponibles de la ENSU con \texttt{map()} sobre la función de lectura escrita en la Sesión 5, consolidación en un solo \texttt{tibble} y producción de una serie de gráficas de percepción de inseguridad por ciudad, exportadas a \texttt{output/} con \texttt{walk()}.}
