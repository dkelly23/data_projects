...
Los ejemplos en clase y los ejercicios del bloque de práctica se construyen sobre estos archivos a lo largo del curso. La EIGH no corresponde a ninguna encuesta real; su estructura imita la de una encuesta de ingreso-gasto de las que produce el INEGI.

La Sesión 5 es la excepción deliberada. Sus temas —control de flujo, vectorización y diseño de funciones— son propiedades del lenguaje y no del dato, de modo que tanto la exposición como los ejercicios operan sobre objetos mínimos declarados en el propio script: así un error de programación no se confunde con un defecto de los archivos. La EIGH regresa en la Sesión 6.
...
La sesión marca la transición del estudiante de consumidor de código a autor de código. Abre con la función como unidad de reuso y desarrolla sobre ella el control de flujo, la vectorización como modo por defecto del lenguaje y el manejo de errores. Es la única sesión que no opera sobre la EIGH: sus temas son propiedades del lenguaje y se ejercitan sobre objetos declarados en el propio programa.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item \textbf{Condicionales.} Sintaxis de \texttt{if} y \texttt{else}; la guardia de una función como primer uso del condicional; \texttt{switch()} para ramas múltiples mutuamente excluyentes, con caso por omisión. \texttt{if} opera sobre escalares: aplicado a un vector es un error desde R 4.2, y antes de esa versión evaluaba solo el primer elemento con \textit{warning}. Distinción: \texttt{if} controla flujo de programa; \texttt{if\_else()} y \texttt{case\_when()} (Sesión 4) construyen variables vectorizadas dentro de \texttt{mutate()}.
    \item \textbf{\textit{Loops}: \texttt{for} y \texttt{while}.} Sintaxis e iteración. Iteración segura sobre secuencias con \texttt{seq\_along()} y \texttt{seq\_len()}. Iteración sobre listas. Pre-asignación de la salida como buena práctica; consecuencias del crecimiento del vector dentro del \textit{loop}. Control fino con \texttt{break} y \texttt{next}. Escenarios en que el \textit{loop} se justifica: operaciones dependientes del estado previo, \textit{side effects} (lectura y escritura de archivos) y número de pasadas desconocido de antemano.
...
\practica{Construcción de una calculadora financiera con funciones propias, sobre datos declarados en el propio script: operaciones vectorizadas sobre una serie de rendimientos, simulación con \textit{loop} de una trayectoria que depende del estado previo, una función con argumentos por defecto, \texttt{switch()} y validación con \texttt{stopifnot()}, un resumen por grupo que recibe la columna como argumento, y captura de fallas con \texttt{tryCatch()} sobre un lote de escenarios. Cierra verificando que la simulación iterativa y la fórmula cerrada dan el mismo resultado. Es la única sesión que no opera sobre la EIGH.}
...
\practica{Cierre del pipeline: lectura de todos los levantamientos disponibles de la EIGH con \texttt{map()} sobre una función de lectura escrita al inicio del bloque, consolidación en un solo \texttt{tibble} y producción de una serie de gráficas de ingreso y gasto por entidad, exportadas a \texttt{output/} con \texttt{walk()}.}
